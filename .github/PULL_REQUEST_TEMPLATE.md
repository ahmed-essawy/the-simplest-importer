...
- [ ] All user-facing strings are wrapped in `__()` / `esc_html__()` with the `the-simplest-importer` text domain
...
- [ ] Functions use the `tsi_` prefix, CSS classes use `tsi-`, constants use `TSI_`
...
2. Go to **Tools → Simplest Importer**.
